...
Một hệ thống quản lý và mượn trả sách Thư viện số cao cấp (Premium Full-Stack Platform) dành riêng cho sinh viên Đại học Bách Khoa Hà Nội (HUST). Dự án kết hợp hài hòa giữa giao diện đồ họa hiện đại mang phong cách Fintech/OLED Luxury ở Frontend và hiệu năng xử lý dữ liệu mạnh mẽ của ngôn ngữ C++ ở Backend (Sử dụng kiến trúc File I/O tùy chỉnh).
...
Hệ thống hoạt động theo mô hình Client-Server độc lập, giao tiếp thông qua giao thức HTTP RESTful API (Sử dụng thư viện `httplib.h`). Cơ sở dữ liệu được quản lý tự động thông qua cấu trúc thư mục `database/` chứa các file Text (`.txt`).

### 1. Cài toàn bộ mã nguồn về IDE
* **Dùng lệnh:**
  ```bash
  git clone [https://github.com/SonDVI/Project-Library-Web-Checkout-books](https://github.com/SonDVI/Project-Library-Web-Checkout-books)

### 2. Khởi động Backend (C++ Server)
